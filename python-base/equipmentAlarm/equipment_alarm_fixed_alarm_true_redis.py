...
logger = getLogger('./logs/equipment_alarm_fixed_alarm_true.log')
# 定义报警规则读取时间间隔（秒）
ALARM_INTERVAL = 10
...
# 定义消费者线程数，报警信息保存线程数，固定值报警测点共 27 个
CUSTOMS_THREAD = 50  # 50
CUSTOMS_THREAD_ALARM = 5  # 5
    ...
    while True:
        try:
            ...
            if point_value is not None and point_timeStamp is not None:
                ...
                point_value_list.append([point_value.decode('utf-8'), real_time, float(point_timeStamp)])
                logger.debug(thread + "--正在处理：{}".format(ruleContext["rule_name"] + "--》" + ruleContext["en_name"] +
                                                         ' 测点值：' + str(point_value.decode('utf-8')) + ' 时间：' +
                                                         real_time + ' 原始状态：' + str(point_alarmFlag)))
            for pv in point_value_list:
                alarmFlag = 0
                ruleContext['point_value'] = pv
                if alarm_value != '' and str(pv[0]) == str(alarm_value):
                    alert_context = '固定报警值，报警值:' + str(pv[0]) + ';报警时间:' + str(pv[1])
            ...
            logger.error('监控处理线程' + thread + " 处理失败：")
            logger.exception(e)
...
def consumersAlarm(alarmQueue, thread):
    ...
    while True:
        try:
            ...
            gp_con.commit()
            logger.debug(thread + "--处理结束：{}".format(' 规则队列剩余：' + ' 报警消息队列剩余' + str(alarmQueue.qsize())))
        except Exception as e:
            logger.error("报警信息处理----------------" + thread)
            logger.exception(e)
            if 'cursor' in e:
