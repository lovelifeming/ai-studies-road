...
logger = getLogger(fileName='./logs/equipment_alarm_warn.log', level=logging.DEBUG)
# 定义报警规则读取时间间隔（秒）
ALARM_INTERVAL = 10
...
def consumers(ruleQueue, alarmQueue, thread):
    ...
    while True:
        try:
            ...
            # logger.info(thread + "--正在处理：{}".format(ruleContext["rule_name"] + "--》" + ruleContext["en_name"]))
            # result = None
            ...
            #               "table_en_time"] + "<='" + end_time + "' order by " + ruleContext["table_en_time"]
            #     result = tsdbCon.query(sql)
            # logger.debug("query result: {}".format(result))
            warn_value = ''
            alarm_value = ''
            proportion_value = 0.1
            ...
            for p in param_info:
                ...
                if p["code"] == "proportion_value":
                    proportion_value = float(p["value"])
            point_value_list = []
            # if table_type == "tsdb":
            ...
            for pv in point_value_list:
                ...
                if alarm_value != '' and pv[0] * proportion_value >= float(alarm_value):
                    alert_context = '大于报警值，报警值:' + str(pv[0] * proportion_value) + ';报警时间:' + str(pv[1])
                    ruleContext['alert_type'] = 2
                    ruleContext['alert_context'] = alert_context
                    alarmFlag = 2
                elif alarm_value != '' and warn_value != '' and float(warn_value) <= pv[0] * proportion_value < float(
                        alarm_value):
                    alert_context = '大于预警值，报警值:' + str(pv[0] * proportion_value) + ';报警时间:' + str(pv[1])
                    ruleContext['alert_type'] = 1
                    ruleContext['alert_context'] = alert_context
                    alarmFlag = 1
...
def consumersAlarm(alarmQueue, thread):
    ...
    while True:
        try:
            ...
            # cur.execute("select device_name,device_id from bd.bd_app_device where device_id in(select device_id from"
            ...
            if 'alert_type' in alarmContent and 'alert_context' in alarmContent:
                ...
                ret = cur.execute(
                    ...
                    % ("'" + device_name + "'", "'" + device_code + "'", str(int(alarmTime)),
                       "'" + str(alert_context) + "'",
                ...
                logger.info("报警信息：{}".format(device_name + "," + device_code + "," + str(alarmTime) + "," +
                                             ...
                                             str(pv) + "," + json.dumps(alarmContent["param_info"])))
...
def main():
    ...
    for i in range(CUSTOMS_THREAD_ALARM):
